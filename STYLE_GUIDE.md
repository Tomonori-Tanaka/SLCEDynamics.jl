...
- **Integrator stages put the strategy first**, then the mutated configuration:
  `_step!(integrator, config, problem, dt, scratch, ntasks)`.
- **Run drivers take `(problem, integrator; keywords)`**; everything that
  configures a run — `dt`, `nsteps`, `temperature`/`kT`, `seed`, intervals,
  observables — is a keyword.

## 3. Package-specific naming

- `prob` is the conventional local for an `LLGProblem`. The reserved locals of
  §1.5 belong to other packages: a scratch buffer here is `scratch` (not `sc`)
  and a run specification is `run_spec` (not `spec`). The current code still uses
  `sc` / `spec` — that is the §1.9 debt, not a licence to add more.
- **Say which gradient.** Three different fields meet inside `_omega`, so a bare
  `g` is never enough: `G` is the SLCE energy gradient `∂E/∂e` (tangent-projected,
  supplied by `energy_gradient!`), `gzee` is the Zeeman gradient `∂E_Z/∂e`
  (deliberately *not* projected), `gth` is the thermal (noise) field.
- `pref` is the per-site prefactor `g/(ħ · magmom · (1 + α²))`; `alpha` is always
  the Gilbert damping and never a step size or a mixing parameter.
- `dt` is a time step in **fs**; `step` is an **absolute** step index. Resume
  bit-identity rests on every per-step effect being a pure function of that
  absolute index, so never shadow `step` with a loop-local offset.
- Filter/thermostat internals name their algebra: `_Biquad` sections are `bq`,
  the state-space twin is `_filter_state_space`. Keep the correspondence with
  `docs/specs/quantum-thermostat.md` explicit in the docstring.

## 4. Docstrings

Public functions state units in the argument list — fs, eV, tesla, kelvin — not
only in prose. This package converts between three unit systems at its boundaries
and a reader skimming the signature must not have to guess which one an argument
is in.
